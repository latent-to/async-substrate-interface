...
def test_old_runtime_calls_natively(substrate):
    # Will not work well for integration test with stubbed data
...
def test_runtime_calls(substrate):
    ...
    batched = substrate.runtime_calls(calls, block_hash=block_hash)
    ...
    individual = [
        substrate.runtime_call(api, method, params=params, block_hash=block_hash)
